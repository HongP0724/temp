...
#define _DUTY_MIN 980   // Servo angle: E degree
...
void setup() {
  ...
  dist_target = 190; // 15.5 cm, the center of the rail
}
...
void loop() {
  ...
  if(event_dist) {
    ...
    dist_filtered = volt_to_distance(ir_sensor_filtered(8, 0.5)); // Replace n with your desired value
...
// Replace the below line with the equation obtained from nonlinear regression analysis
  return  680 + (-4.39) * a_value + 0.0103* a_value * a_value + (-7.96E-06) * a_value * a_value * a_value;
}
