...
#define _DUTY_MAX 2360  // Servo angle: D degree
...
// PID parameters
#define _KP_PLUS 3.30 // proportional gain
#define _KP_MINUS 3.25 // proportional gain
...
void setup()
{
  ...
  dist_target = 210; // the center of the rail (unit: mm)
...
void loop()
{
  ...
  if(event_dist) {
    ...
    dist_filtered = volt_to_distance(ir_sensor_filtered(6, 0.5));
    dist_ema = _EMA_ALPHA * dist_ema + (1.0 - _EMA_ALPHA) * dist_filtered;

    // Update PID control variables
    error_curr = dist_target - dist_ema;
    pterm = ((error_curr > 0) ? _KP_PLUS : _KP_MINUS) * error_curr;
    control = pterm;
...
{
  return  680 + (-4.39) * a_value + 0.0103* a_value * a_value + (-7.96E-06) * a_value * a_value * a_value; // Replace this with the equation obtained from nonlinear regression analysis
}
